...
const NoComments = () => {
    return (
        <h2>No Comments Yet. Be the first to post!</h2>
    )
}
...
const DrinkComments = () => {
    ...
    const handleAddComment = async e => {
        e.preventDefault()
        try {
            await axios.post(`/api/drinks/comments/${drink._id}`,
    ...
    const getComments = async () => {
        try {
            const {data} = await axios.get(`/api/drinks/comments/${drink._id}`)
    ...
    useEffect(() => {getComments()}, [subscription])

    return (
        <div>
            {signedIn ? (
                <form onSubmit={handleAddComment}>
                    <textarea placeholder="Write comment here..." value={newComment} onChange={e => setNewComment(e.target.value)}/>
                    <br />
                    <Button size="small" type="submit" color="default" variant="contained">Post Comment</Button>
                </form>
            ): (
                <div>
                    <textarea disabled placeholder="Please sign in to post comments"/>
                </div>
            )}
            {comments.map((
                {
                ...
                <div className="user-comment-container" key={_id}>
                    <Avatar variant="outlined" src={picture_url_thumbnail} alt='profile' />
                    <div>
